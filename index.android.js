...
async function getTopStoriesFromApi() {
  try {
    ...
    let stories = await getStoriesFromIds(responseJson, 20);
    return stories;
  } catch (error) {
    console.error(error);
  }
          ...
          renderRow={(rowData) => (
            <TouchableHighlight
              ...
              <View>
                ...
                <Text style={styles.details}>{rowData.score} points by {rowData.by} {moment(+rowData.time * 1000).fromNow()} | {rowData.descendants} comments</Text>
              </View>
            </TouchableHighlight>
          )}
          refreshControl={
